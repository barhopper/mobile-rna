...
import { getBar, getMyCheckins, searchForBars, submitCheckout } from './actions/bars'
// ErrorBoundary
  ...
  async function schedulePushNotification(bar, type) {
    ...
    if (!hasNotification) {
      ...
      await Notifications.scheduleNotificationAsync({
        content: {
          title: `Looks like you ${type === 'CHECKIN' ? 'are near' : 'left'
            } ${barName}`,
          ...
          getMyCheckins(currentUser.uid).then(checkins => {
            ...
            searchForBars('bars', 0.03, [
              ...
              bars.map(b => {
                ...
                if (!amICheckedIn) {
                  console.log('Schedule Push Notification Checkin')
